          ...
          <div ref={stepsRef} className="space-y-12 md:space-y-24">
            {steps.map((step, index) => (
              ...
              >
                {/* Text Content Side */}
                <div 
                  className={`flex-1 ${index % 2 === 0 ? 'md:text-right' : 'md:text-left'} text-center fill-mode-both ${stepsInView ? `animate-in fade-in ${index % 2 === 0 ? 'md:slide-in-from-left-12' : 'md:slide-in-from-right-12'} duration-[2000ms] opacity-100` : 'opacity-0'}`}
                ...
                {/* Center Circle */}
                <div 
                  className={`relative flex items-center justify-center fill-mode-both ${stepsInView ? 'animate-in fade-in zoom-in duration-[1000ms] opacity-100' : 'opacity-0'}`}
                  style={{ animationDelay: `${index * 1000 + 200}ms` }}
                >
                ...
                {/* Image Side */}
                <div 
                  className={`flex-1 fill-mode-both ${stepsInView ? `animate-in fade-in ${index % 2 === 0 ? 'md:slide-in-from-right-12' : 'md:slide-in-from-left-12'} duration-[2000ms] opacity-100` : 'opacity-0'}`}
                  style={{ animationDelay: `${index * 1000 + 400}ms` }}
                >
